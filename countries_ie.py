...
def get_prime_minister_info(table,g,countryName):
    ...
    if len(row) != 0:
        ...
        if len(primeLink) != 0:
            primeMinisterEntity = rdflib.URIRef('http://en.wikipedia.org/wiki/' + primeLink[0].replace(" ", "_").replace("\n", "").replace("\t", "").lower())
            ...
            primeLink = table[0].xpath("//table//tbody//tr[contains(th//a//text(), 'Prime Minister') and not(contains(th//a//text(), 'Deputy'))]//td//text()")
            if primeLink[0] != "Vacant":
                primeMinisterEntity = rdflib.URIRef('http://en.wikipedia.org/wiki/' + primeLink[0].replace(" ", "_").replace("\n", "").replace("\t", "").lower())
                isPrimeMinisterOf = rdflib.URIRef('http://en.wikipedia.org/wiki/isPrimeMinisterOf')
                g.add((primeMinisterEntity, isPrimeMinisterOf, countryName))
